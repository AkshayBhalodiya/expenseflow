...
class ApiClient {
  async request<T>(endpoint: string, options: ApiOptions = {}): Promise<T> {
    ...
    const token = options.token || tokenStorage.getAccessToken();
  ...
  private async refreshAccessToken(): Promise<boolean> {
    ...
    try {
      ...
      if (res.ok && data.data?.accessToken) {
        await tokenStorage.setTokens(data.data.accessToken);
        return true;
      }
    } catch {
      // ignore
  ...
  scanReceipt = (uri: string, fileName: string, mimeType: string, autoCreate = false) => {
    const form = new FormData();
    form.append('receipt', { uri, name: fileName, type: mimeType } as unknown as Blob);
